Confirmed — `fastify-plugin` is not a direct dependency. Now I have everything I need to produce the revised plan.
...
| `fastify-plugin` | **Needs install** | Not in `packages/server/package.json` — only present as a transitive dependency of `@fastify/jwt` and `@fastify/cors`, which is not safe to rely on |
...
1. **Install `fastify-plugin` as a direct dependency** — Run `npm install fastify-plugin -w packages/server`. This package is required by both plugin files (D1 and D2) to break Fastify's encapsulation so that decorators like `app.jwt` are accessible at the app level. It must be a direct dependency, not relied upon as a transitive dependency of `@fastify/jwt` or `@fastify/cors`.
