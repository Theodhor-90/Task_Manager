I have enough context from the master plan to specify version ranges. Let me also check if there's a Tailwind v4 consideration — the master plan says "Tailwind CSS" without specifying a version. Let me proceed with drafting the plan based on the conventions I've observed.

Now I have all the information I need. Here is the complete implementation plan:

---

# Implementation Plan — t04: Client package scaffold
...
| 2 | `packages/client/tsconfig.json` | Create | TypeScript config for Vite/React — cannot simply extend `../../tsconfig.base.json` |
...
- **`build` uses `tsc -b && vite build`** — type-checks first with TypeScript, then builds with Vite. `tsc -b` (build mode) respects project references and is the Vite-recommended approach for type checking. Vite itself uses esbuild for transpilation (no type checking).
...
- **React 19** — the master plan says "React 18+". React 19 is the current stable version and is backwards-compatible. Using `^19.0.0` to get the latest.
...
- **Does NOT extend `../../tsconfig.base.json`** — the base config has `module: "Node16"`, `moduleResolution: "Node16"`, `verbatimModuleSyntax: true`, `declaration: true`, `declarationMap: true`, `sourceMap: true`, and `outDir`/`rootDir` are expected by extending packages. These are incompatible with a Vite/React project:
  - `module: "Node16"` requires `.js` extensions on imports — Vite resolves imports without extensions.
  - `verbatimModuleSyntax: true` requires `import type` syntax and forbids `import React from "react"` style defaults — many React ecosystem packages (react-router-dom, etc.) rely on default imports that don't have explicit `type` markers.
...
- **`noEmit: true`** — TypeScript is used only for type checking. Vite handles transpilation via esbuild. `tsc -b` in the build script type-checks without emitting.
...
- **Carries forward compatible options from base** — `target`, `strict`, `esModuleInterop`, `skipLibCheck`, `forceConsistentCasingInFileNames`, `isolatedModules`, `resolveJsonModule` are all preserved from the base config for consistency.
...
- **Empty `theme.extend`** — no custom theme overrides yet. Will be extended when UI components need custom colors, spacing, etc.
...
- **`tailwindcss` + `autoprefixer`** — the two PostCSS plugins required by Tailwind v3. Autoprefixer adds vendor prefixes for cross-browser compatibility.
...
- **`<script type="module" src="/src/main.tsx">`** — Vite uses the HTML file as the entry point. The `type="module"` attribute is required for ESM. Vite handles `.tsx` files natively.
- **`<div id="root">`** — standard React mount target.
- **Title "TaskBoard"** — matches the project name from the master plan.
- **No favicon or external resources** — minimal scaffold. Assets can be added later.
...
- **`createRoot`** — React 19 API (same as React 18's concurrent root API).
- **`StrictMode`** — enables additional development warnings and double-rendering checks. Recommended for all React apps.
- **`import { App } from "./App.tsx"`** — named import, follows project convention of named exports. The `.tsx` extension is included because Vite resolves it directly (unlike Node16 module resolution which requires `.js`).
- **`import "./index.css"`** — imports the Tailwind CSS entry point. Vite processes this through PostCSS.
- **Null check on root element** — throws a clear error instead of silently failing if the DOM element is missing.
...
- **`BrowserRouter`** — uses the HTML5 History API for clean URLs. Standard for SPAs.
- **Three placeholder routes** — matches the master plan Section 6.1:
  - `/login` — Login page
  - `/` — Dashboard (project list)
  - `/projects/:id/board` — Kanban board for a project
- **Inline placeholder components** — `LoginPage`, `DashboardPage`, `BoardPage` are simple `<h1>` placeholders. They live in the same file for now because they have no logic. They will be extracted to separate files in `src/pages/` when real implementations are added in Milestones 3–4.
- **Named export `App`** — follows project convention. No default exports.
- **No `import React`** — not needed with `jsx: "react-jsx"` (automatic runtime).
- **Route params** — `/projects/:id/board` uses `:id` parameter matching the master plan's URL pattern.
...
- **Three Tailwind directives** — the standard Tailwind v3 entry point. These are processed by PostCSS at build time and replaced with Tailwind's generated CSS.
- **No custom CSS** — the master plan says "utility-first, minimal custom CSS." Custom styles will be added only when needed.
...
This task creates a scaffold — there is no runtime logic to unit-test. The placeholder components render static HTML. Verification is structural and functional (does it compile, does it serve, do routes work).

| # | Check | Method | Expected Result |
|---|-------|--------|-----------------|
| 1 | Package compiles without TS errors | `npx tsc -b -p packages/client/tsconfig.json --noEmit` | Exit code 0, no errors |
| 2 | `package.json` has correct name | Read file | `"@taskboard/client"` |
| 3 | `package.json` has `dev` script | Read file | `"vite"` |
| 4 | `package.json` has `build` script | Read file | `"tsc -b && vite build"` |
...
| 14 | `vite.config.ts` has React plugin | Read file | `react()` in plugins array |
| 15 | `vite.config.ts` has `/api` proxy to port 3001 | Read file | Proxy config present |
| 16 | `tailwind.config.js` has content globs | Read file | `./src/**/*.{js,ts,jsx,tsx}` |
| 17 | `postcss.config.js` has tailwindcss plugin | Read file | Present |
| 18 | `index.html` has root div and module script | Read file | `<div id="root">` and `<script type="module">` |
| 19 | `main.tsx` creates React root and renders App | Read file | `createRoot` and `<App />` |
| 20 | `App.tsx` has all 3 routes | Read file | `/login`, `/`, `/projects/:id/board` |
| 21 | `index.css` has Tailwind directives | Read file | `@tailwind base`, `@tailwind components`, `@tailwind utilities` |
| 22 | `npm install` succeeds from root | Run command | Exit code 0 |
| 23 | `npm run build -w @taskboard/client` succeeds | Run command | Exit code 0, `dist/` populated |
| 24 | Vite dev server starts | Run `npm run dev -w @taskboard/client` briefly | Starts on port 5173 |
| 25 | npm workspace resolves package | `npm ls @taskboard/client` | Package listed, no errors |
...
node --input-type=module -e "
  ...
  console.assert(p.scripts.build === 'tsc -b && vite build', 'build script mismatch');
...
# 6. Verify tsconfig.json has Vite-compatible settings
node --input-type=module -e "
  import { readFileSync } from 'fs';
...
timeout 5 npx vite --config packages/client/vite.config.ts 2>&1 || true
