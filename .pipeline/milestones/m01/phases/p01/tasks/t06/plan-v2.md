...
- **No `Object.isFrozen` tests** — `as const` is a TypeScript-only construct that narrows the type at compile time but does NOT freeze the array at runtime. `Object.isFrozen(PRIORITIES)` would return `false`, causing the test to fail. The `readonly` type annotation prevents mutation at compile time (TypeScript will error on `.push()` etc.), but that's a type-system guarantee, not a runtime one. Verifying length and exact values is sufficient.
...
- **4 tests total** — two per constant. Trivial but meaningful: they verify the exact shape that downstream packages depend on.
...
|---|-------|--------|-----------------|
| 1 | Shared package tests pass | `npm run test -w @taskboard/shared` | 4 tests pass, 0 failures |
| 2 | Server package tests pass | `npm run test -w @taskboard/server` | 3 tests pass, 0 failures |
| 3 | Root test command runs both packages | `npm run test` | 7 total tests pass across 2 packages |
...
| 5 | `Object.isFrozen` test — will `as const` arrays pass? | **No — removed** | `as const` is a TypeScript-only construct that narrows types at compile time but does NOT freeze arrays at runtime. `Object.isFrozen(PRIORITIES)` returns `false`. The `readonly` type annotation prevents mutation at compile time via TypeScript's type checker, not at runtime. Testing length and exact values is sufficient. |
...
|---|---------------|--------------|
| 1 | `Object.isFrozen` tests will fail — `as const` does NOT freeze arrays at runtime | Removed the two `Object.isFrozen` tests (one for `PRIORITIES`, one for `DEFAULT_COLUMNS`) from §3.3. Updated test count from 6 to 4 in §3.3. Updated §5 Test Plan rows #1 and #3 to reflect new test counts (4 shared tests, 7 total). Updated tiebreaker decision #5 to explain why `Object.isFrozen` was removed. |