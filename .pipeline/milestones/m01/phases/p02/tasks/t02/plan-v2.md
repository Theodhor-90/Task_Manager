Now I have all the context I need. Let me summarize the feedback and produce the revised plan:

**Feedback points to address:**
1. **Remove conversational preamble** on line 1 ("Now I have the complete picture. Let me produce the implementation plan.")
2. **Fix misleading dependency on shared package**: The plan says it depends on `User` interface from `packages/shared/src/types/index.ts`, but that source file doesn't exist on this branch (only compiled `dist/` exists). The implementation code doesn't actually import from shared. Clarify that the shared `User` interface is a *design reference* only, not an import.

Everything else was confirmed as sound. Here's the revised plan:
...
- **Standalone hash/verify functions** (not pre-save hooks): The task spec explicitly requires standalone utility functions. This gives callers explicit control — the seed script and auth routes call `hashPassword` before setting `passwordHash`, which avoids double-hashing bugs and makes testing straightforward
- **`bcryptjs`** (not native `bcrypt`): Pure JavaScript implementation avoids native compilation issues. The API is identical. The milestone spec-v1 explicitly recommends this approach
...
No dedicated test file is required for this task. The task spec lists verification criteria (functional behavior), and comprehensive model tests are defined in t08 (`packages/server/test/models/user.model.test.ts`). The t08 spec explicitly lists the following test cases for the User model:
