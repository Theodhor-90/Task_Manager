Now I have the complete picture. Let me produce the implementation plan.
...
### Prerequisites
- **Phase 1 must be merged** into the working branch — specifically `packages/shared/src/types/index.ts` (provides the `User` interface), `packages/server/src/config.ts`, the server package scaffold, and Vitest configuration
...
### Existing Code Used
- `User` interface from `packages/shared/src/types/index.ts` — defines the entity shape (`_id`, `email`, `passwordHash`, `name`, `createdAt`, `updatedAt`)
...
**Key decisions**:
- **`UserDocument` interface**: Extends Mongoose `Document` to provide typed access to fields. This is the Mongoose-level document type; the shared `User` interface in `@taskboard/shared` is the API/transport-level type
