I need write permission to save the plan file. The plan is complete and ready to be written to `.pipeline/milestones/m04/phases/p04/tasks/t10/plan.md`. Could you grant write permission so I can save it?
...
6. **`components/__tests__/task-detail-panel.test.tsx`** — verifies:
   ...
   - `CommentList` integration (rendered with taskId)
...
|---|-----------|-------------|
| 1 | CommentList renders comments in chronological order with author name and timestamp | `task-detail-panel.test.tsx`, `comment-list.tsx` structure |
| 2 | New comments can be added and appear immediately | `comment-form.tsx` → `createComment` → `onCommentAdded` |
| 3 | Comments can be edited inline and deleted with confirmation | `comment-list.tsx` edit/delete handlers |
| 4 | Labels can be created with name and hex color | `label-manager.test.tsx` test 5 |
| 5 | Labels can be edited and deleted; deleting removes from all tasks | `label-manager.test.tsx` tests 9, 12; `board-context.test.tsx` |
| 6 | LabelPicker shows all labels with checkboxes; toggling attaches/detaches | `label-picker.test.tsx` tests 3-7 |
| 7 | TaskCard renders label dots in actual colors | `task-card.test.tsx` color tests |
| 8 | FilterBar renders with label, priority, and due date controls | `filter-bar.test.tsx` test 1; `board-view.test.tsx` |
| 9 | Label filter shows only tasks with selected labels | `board-view.test.tsx` label filter test |
| 10 | Priority filter shows only tasks with selected priorities | `board-view.test.tsx` priority filter test |
| 11 | Due date filter shows only tasks within range | `board-view.test.tsx` due date filter test |
| 12 | Filters combine with AND logic; clearing restores full board | `board-view.test.tsx` combined + clear tests |
| 13 | Intra-column reordering disabled when filtering | `board-view.test.tsx` reorder guard test |
| 14 | All interactions persist to server via REST API | Server route tests + client API call assertions |
...
grep -n "CommentList\|LabelPicker" packages/client/src/components/task-detail-panel.tsx
