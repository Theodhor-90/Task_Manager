Good — vitest with vite handles ESM imports from react-markdown natively, so no special config needed. Now I have all the context. Let me write the plan.
...
- **Existing infrastructure**:
  ...
  - `react-markdown` v10 — exports `Markdown` as a named export; accepts `children` (string) prop for markdown content
- **Not available**: `@tailwindcss/typography` is not installed, so the `prose` class is unavailable. Markdown preview styling must use manual Tailwind utility classes applied via `react-markdown`'s `components` prop or a wrapper div with custom CSS classes.
...
Note: `react-markdown` v10 exports `Markdown` as a named export (not a default export, not `ReactMarkdown`).
...
- **Tab bar**: Two buttons styled as tabs with a bottom border. The active tab has a blue bottom border and blue text; inactive tabs have gray text. This follows a standard tab pattern. The tabs sit inside a `flex` container with a bottom border.
...
- **`Markdown` component**: `react-markdown` v10's `Markdown` component takes the markdown string as `children`.

#### 3.1.6 Markdown styling approach

Since `@tailwindcss/typography` is not installed, the rendered markdown will use browser defaults for elements like `<h1>`, `<p>`, `<ul>`, `<a>`, `<code>`, etc. To provide basic readable formatting without installing a new plugin, add a simple `prose-custom` class with manual styling.

Add to `packages/client/src/index.css` after the Tailwind directives:

**Actually, the simpler approach**: Rather than adding global CSS, use `react-markdown`'s `components` prop to apply inline Tailwind classes to each element. However, this adds significant boilerplate for a basic feature. The simplest approach that achieves readable markdown output is to add minimal CSS.

**Decision**: Add a small CSS block to `packages/client/src/index.css` for the `prose-custom` class that styles basic markdown elements. This is minimal, non-intrusive, and avoids a new npm dependency.
...
The existing Escape key handler (section 3.1.5 of the Task 4 plan) currently handles two cases: cancel title editing, or close the panel. We do **not** need to add a third case for description editing because:
...
- The textarea blur event will fire when the panel closes (since the panel unmounts), triggering `handleDescriptionBlur` — but this is fine because the `cancelled` flag in the data loading effect doesn't apply here. The save will still fire correctly since the task state is still available at that point.

**However**, there is a subtlety: if the user presses Escape while the textarea is focused, the document keydown handler fires `onClose()`, which unmounts the component. The `onBlur` on the textarea will fire during unmount, calling `handleDescriptionBlur`, which will try to call `updateTask` on an unmounting component. This is harmless (React handles setState on unmounted components gracefully in React 19), but to be safe and avoid unnecessary API calls on close, we should **not** save on blur when the panel is closing.

**Simple solution**: No change needed. The `handleDescriptionBlur` function checks `if (!task) return;` — but `task` is still set at unmount time, so it will attempt the save. This is actually acceptable behavior: saving the description on panel close is a reasonable UX. The user expects their edits to persist. If we explicitly wanted to discard on Escape, we'd need a ref to track "closing" state, but the spec says "description saves on blur," and closing the panel triggers blur.
...
Add a mock for `react-markdown` at the top of the file, alongside the existing mocks:
...
The `Markdown` component from `react-markdown` v10 accepts:
...
1. Add `vi.mock("react-markdown")` with a simple mock that renders content in a `data-testid="markdown-preview"` div
...
2. **Step 2**: Modify `packages/client/src/components/task-detail-panel.tsx`:
   - Add `import { Markdown } from "react-markdown";`
...
3. **Step 3**: Modify `packages/client/src/components/__tests__/task-detail-panel.test.tsx`:
   - Add `vi.mock("react-markdown")` mock
...
```bash
# 1. Verify the Markdown import exists
grep "import { Markdown } from" packages/client/src/components/task-detail-panel.tsx
