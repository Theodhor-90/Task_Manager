Now I have a full picture of the codebase. The task requires adding `GET /:id` and `PUT /:id` handlers. The test-double needs `findOneAndUpdate`. Let me also verify the `_id` format in the test-double to understand how ObjectId validation will work.
...
| # | File | Action | Purpose |
|---|------|--------|---------|
| 1 | `packages/server/test/helpers/mongoose.test-double.ts` | **Modify** | Add `findOneAndUpdate` method and `deleteOne` method to the model object |
...
- **Mongoose test-double gap**: The test-double does not currently support `findOneAndUpdate`. This method is required by the `PUT /:id` handler. It also does not support `deleteOne`, which is referenced in phase spec task 3 but not strictly needed here — however, adding it now alongside `findOneAndUpdate` is minimal effort and avoids a gap later.
- **ObjectId validation**: The test-double exports `Types.ObjectId` with a static `isValid()` method. In the route handler, we need to import `mongoose` (which resolves to the test-double in tests) to access `mongoose.Types.ObjectId.isValid()`. However, since the route file currently imports from `@taskboard/shared` and `../models/index.js` but not `mongoose` directly, and since the real mongoose is a production dependency, we need to import it. The test-double already exports `Types` with `ObjectId.isValid`.

## 3. Implementation Details

### 3.1 Mongoose test-double — Add `findOneAndUpdate` and `deleteOne`
...
async findOneAndUpdate(
  ...
  return options.new ? doc : doc; // simplified — always returns the doc
},
```

**Add `deleteOne` inside the `model()` function's return object** (after `deleteMany`):
...
**New type alias for `findOneAndUpdate` support**:
```typescript
...
This type-cast pattern matches the existing `FindProjectsModel` pattern used for the `find()` call — the test-double's model object doesn't match Mongoose's type exactly, so we cast through `unknown`.
...
app.put("/:id", async (request, reply) => {
  ...
  const updatedProject = await (ProjectModel as unknown as UpdateProjectModel)
    .findOneAndUpdate(
...
- Create a project, then request `GET /api/projects/:id` without auth header
- Expect: 401
...
**Test 11: returns 400 when name is empty string**
- Create a project
- Send `PUT /api/projects/:id` with `{ name: "" }` and auth header
- Expect: 400

**Test 12: returns 400 when name is whitespace only**
- Create a project
- Send `PUT /api/projects/:id` with `{ name: "   " }` and auth header
- Expect: 400

**Test 13: returns 401 without auth token**
- Send `PUT /api/projects/:id` with `{ name: "Test" }` and no auth header
- Expect: 401
- Assert: `body.error === "Unauthorized"`

## 6. Implementation Order

1. **Extend mongoose test-double** — Add `findOneAndUpdate` and `deleteOne` methods to the model object in `packages/server/test/helpers/mongoose.test-double.ts`. This unblocks the `PUT /:id` handler from being testable.

2. **Add route handlers to `project.routes.ts`** — Import `mongoose`, add `isValidObjectId` helper, add `isValidUpdateProjectBody` validator, add `UpdateProjectModel` type, implement `GET /:id` and `PUT /:id` handlers.

3. **Add integration tests to `project.routes.test.ts`** — Add two new `describe` blocks with all 13 tests.
