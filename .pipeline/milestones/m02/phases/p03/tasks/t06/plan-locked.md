Now I have all the context I need. Let me produce the revised plan that addresses every point in the feedback.
...
| # | File | Action | Purpose |
|---|------|--------|---------|
| 1 | `packages/server/test/helpers/mongoose.test-double.ts` | Modify | Extend the in-memory test double to support `.populate()` on `findOne`, `$ne` operator in `matches`, `$gte` operator in `matches`, and array-contains matching for scalar-vs-array comparisons — all required by the task route handlers |
...
The current test double lacks four features required by the task routes:
...
**Location**: Inside the `matches` function (lines 89-106), in the operator-checking `if` block, after the existing `$in` check (line 101).
...
**Location**: Same operator-checking block in `matches`, after the `$ne` handler.
...
#### 3.1.3 Array-contains matching for scalar filter values (CRITICAL)

The task list endpoint uses `filter.labels = label` (line 195 of `task.routes.ts`) to filter tasks by label ID. In real Mongoose, querying `{ labels: "someId" }` on an array field implicitly checks if any element in the array matches (array-contains semantics). However, the current test double's `matches` function at line 104 does `normalizeForCompare(doc[key]) === normalizeForCompare(value)` — when `doc.labels` is an array like `["abc123"]`, `normalizeForCompare` returns a new array `["abc123"]`, which will never `===` the string `"abc123"`. This means **all label filter tests would fail** without this fix.

**Location**: Replace the final `return` statement at line 104 of the `matches` function.
...
This mimics Mongoose's implicit array-contains behavior: when the document field is an array and the filter value is a scalar, check if any element in the array matches the filter value.

#### 3.1.4 `.populate()` chain method on `findOne`

The `GET /api/tasks/:id` handler chains `.findOne({ _id: id }).populate("labels")`. The current `findOne` (line 315-317) returns a plain Promise with no `.populate()` method.
...
**Note on imports**: `TaskModel`, `CommentModel`, and `LabelModel` are imported for direct data setup (creating labels and comments that are not yet available through API). Position verification is done exclusively through the API `getTasks` helper, which is sufficient for all position assertions because the GET list endpoint returns tasks with their position field. No `BoardModel` import is needed since all data retrieval goes through the API helpers.

#### 3.2.2 Shared Infrastructure

Copy the exact same patterns from `board.routes.test.ts`:
...
| 6 | Sorts by createdAt ascending | `createProject`, create two tasks with a `await new Promise(r => setTimeout(r, 10))` delay between them to guarantee distinct timestamps | GET `?sort=createdAt&order=asc` | First task has earlier createdAt than second |
| 7 | Sorts by createdAt descending | Same setup (tasks created with delay) | GET `?sort=createdAt&order=desc` | First task has later createdAt than second |
...
| 10 | Combines filter and sort | `createProject`, create tasks with varying status and createdAt (with delays) | GET `?status=To+Do&sort=createdAt&order=desc` | Only "To Do" tasks, sorted descending by createdAt |
...
**Note on createdAt sort tests**: The test double's `create` method assigns `new Date()` at call time (line 300 of `mongoose.test-double.ts`). Tasks created in rapid succession may receive identical timestamps. To guarantee distinct timestamps for sort verification, insert a `await new Promise(r => setTimeout(r, 10))` delay between task creation calls in tests #6, #7, and #10. This is a minimal 10ms pause that reliably produces different `Date` values.

**Suite 3: `describe("GET /api/tasks/:id")`**

| # | Test | Setup | Action | Assertions |
|---|------|-------|--------|------------|
| 1 | Returns task with populated labels | `createProject`, create label via `LabelModel.create({ name: "Bug", color: "#ef4444", project: projectId })`, create task with `labels: [normalizeId(label._id)]` | GET `/api/tasks/:taskId` | Status 200; `data.labels[0]` is an object with `name === "Bug"`, `color === "#ef4444"`, `_id` defined (not just an ID string) |
...
| 2 | Cascade deletes associated comments | `createProject`, create task, create comment via `CommentModel.create({ body: "test", task: taskId, author: userId })` referencing the task | DELETE task | Status 200; verify via `CommentModel.countDocuments({ task: taskId })` returns 0 |
| 3 | Reindexes positions of remaining tasks | `createProject`, create 3 tasks in "To Do" (positions 0, 1, 2) | DELETE the middle task (position 1) | Status 200; fetch remaining tasks via `getTasks(boardId, "status=To+Do")` sorted by position -> positions are 0 and 1 (contiguous) |
...
| 2 | Reindexes source column after move out | Create 3 tasks in "To Do" (positions 0, 1, 2), move task at position 1 to "In Progress" | Fetch tasks via `getTasks(boardId, "status=To+Do")` | Two remaining tasks have positions 0 and 1 |
| 3 | Reindexes destination column after move in | Create 2 tasks in "In Progress" (positions 0, 1), move task from "To Do" to "In Progress" at position 0 | Fetch tasks via `getTasks(boardId, "status=In+Progress")` | Three tasks with positions 0 (moved), 1, 2 |
...
Since the Labels API (Phase 4) is not yet implemented, labels are created directly via `LabelModel.create({ name: "Bug", color: "#ef4444", project: projectId })`. The label's `_id` is then used in task creation bodies and for filter queries. The `_id` must be normalized with `normalizeId()` to get a string suitable for use in API request bodies and query parameters.

### Position Verification

For move and delete tests, after the operation, fetch ALL tasks for the board (optionally filtered by status) using the `getTasks` API helper, sort them by position, and verify contiguous 0-based positions. This approach uses the API endpoint exclusively (no direct model access needed), which is consistent with integration test philosophy and sufficient for all position assertions since the GET list endpoint returns tasks with their position field.

### Comment Cascade Verification

After deleting a task with comments, use `CommentModel.countDocuments({ task: taskId })` directly to verify all comments were removed. The `taskId` should be passed as a string (normalizeId) so the test double's `matches` function can compare correctly.

### Timestamp Distinctness for Sort Tests

The test double's `create` method assigns `new Date()` at call time. Tests that verify createdAt sorting (Suite 2 tests #6, #7, #10) must insert a `await new Promise(r => setTimeout(r, 10))` delay between task creation calls to guarantee distinct timestamps. This 10ms pause is minimal but reliably produces different `Date` values.

## 6. Implementation Order

1. **Extend `mongoose.test-double.ts`**
   - Add `$ne` operator support in the `matches` function (after the `$in` handler)
   - Add `$gte` operator support in the `matches` function (after `$ne`)
   - Add array-contains matching: replace the final scalar comparison in `matches` with logic that checks if the document field is an array containing the filter value
