Now I have a complete understanding of the existing codebase. Let me produce the implementation plan.
...
- When `onDismiss` is provided, render a dismiss button: an `<button>` with an `×` character (or SVG X icon), styled `text-red-500 hover:text-red-700`, with `aria-label="Dismiss"`.
...
- Test framework: Vitest + React Testing Library (already in the project's test stack)
...
1. **`loading-spinner.tsx`** — No dependencies on other new components. Simplest component, establishes the `ui/` directory.
2. **`error-message.tsx`** — No dependencies on other new components. Second simplest.
3. **`modal.tsx`** — No dependencies on other new components, but more complex (portal, escape key, click-outside).
4. **`confirm-dialog.tsx`** — Depends on `Modal`, so must come after it.
5. **Tests** — Write tests for all four components after they are created. Tests can be written in parallel since they are independent, but all source files must exist first.
...
# 3. Run the dev server to verify no runtime errors
npm run dev

# 4. Run tests (once test files are written)
cd packages/client && npx vitest run src/components/ui/__tests__/

# 5. Verify named exports are correct (quick import check via tsc)
cd packages/client && npx tsc --noEmit
```