...
import { makeDefaultPhaseState, makeDefaultTaskState, } from "./state.js";
// ── Milestone Scaffolding (bootstrap from master plan) ──────
export function scaffoldMilestones() {
    ...
    const result = callAgentStructured(scaffold.agent, prompt, scaffold.options);
...
export function scaffoldPhases(milestoneDir, state, milestoneId) {
...
export function scaffoldTasks(phaseDir, state, milestoneId, phaseId) {
    ...
    for (const item of result.items) {
        ...
        log(`${milestoneId}/${phaseId}/${item.id}`, "scaffold", `Created task seed spec: ${item.title}`);
    }
}
