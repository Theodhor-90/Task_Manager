...
// ── Parser ───────────────────────────────────────────────────
export function parseDecision(raw, schemaName) {
    ...
    if (lowerRaw.includes("needs_revision") || lowerRaw.includes("needs revision")) {
        log("", "parse", `WARNING: Fell back to text matching for ${schemaName} — detected "needs_revision"`);
        return { verdict: "needs_revision", feedback: raw };
    }
    // Strategy 4: Total failure — treat as rejection
    log("", "parse", `WARNING: All parse strategies failed for ${schemaName}. Treating as needs_revision.`);
    return {
...
function isValidDecision(obj) {
    if (typeof obj !== "object" || obj === null)
        return false;
    const d = obj;
    if (d.verdict !== "approved" && d.verdict !== "needs_revision")
        return false;
