...
import { loadState, saveState, sortedKeys, makeDefaultMilestoneState, makeDefaultPhaseState, makeDefaultTaskState, } from "./state.js";
                ...
                if (existsSync(tasksDir)) {
                    for (const taskId of readdirSync(tasksDir).sort()) {
                        ...
                        if (!isDirectory(taskDir))
                            continue;
                        if (!existsSync(join(taskDir, "spec.md")))
                            continue;
...
function runStatus() {
    ...
    for (const milestoneId of sortedKeys(state.milestones)) {
        ...
        for (const phaseId of sortedKeys(milestone.phases)) {
            ...
            for (const taskId of sortedKeys(phase.tasks)) {
                ...
                if (task.planning.tiebreakerUsed)
                    parts.push("tiebreaker: plan");
                if (task.implementation.tiebreakerUsed)
                    parts.push("tiebreaker: impl");
    ...
    console.log(`Progress: ${completedMilestones}/${totalMilestones} milestones, ` +
...
function runReset() {
    ...
    if (parts.length === 1) {
        ...
        resetPhaseState(phase);
        cleanArtifacts(join(CONFIG.pipelineDir, "milestones", milestoneId, "phases", phaseId), ["spec-v", "feedback-v", "spec-locked.md", "tiebreak"]);
        ...
        const taskDir = join(CONFIG.pipelineDir, "milestones", milestoneId, "phases", phaseId, "tasks", taskId);
        if (targetStatus === "pending") {
...
// ── Helpers ──────────────────────────────────────────────────
function resetPhaseState(phase) {
...
function cleanArtifacts(dir, prefixes) {
    if (!existsSync(dir))
        return;
