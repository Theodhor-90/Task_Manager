...
import { checkpoint, saveState, sortedKeys, transitionMilestone, transitionPhase, transitionTask, getMilestone, getPhase, getTask, } from "./state.js";
...
export class PhaseCompleteSignal extends Error {
    milestoneId;
    phaseId;
    constructor(milestoneId, phaseId) {
        ...
        for (const phaseId of sortedKeys(milestone.phases)) {
            ...
            for (const taskId of sortedKeys(phase.tasks)) {
                ...
                if (task.status === "pending" || task.status === "planning") {
                    ...
                    const completedSiblings = buildCompletedSiblingsSection(state, milestoneId, phaseId, taskId);
                    const result = runIterationCycle({
                    ...
                    checkpoint(statePath, state, `task ${milestoneId}/${phaseId}/${taskId} plan locked`);
                }
                ...
                if (task.status === "plan_locked" || task.status === "implementing") {
                    if (task.status === "plan_locked") {
                        ...
                        checkpoint(statePath, state, `task ${milestoneId}/${phaseId}/${taskId} → implementing`);
                    }
                    const result = runIterationCycle({
                        label: `${milestoneId}/${phaseId}/${taskId}`,
                        ...
                        templateVars: {
                            ...
                            SPEC_PATH: join(taskDir, "spec.md"),
                            PHASE_SPEC_PATH: join(milestoneDir, "phases", phaseId, "spec-locked.md"),
                        },
                        artifactNames: {
                    ...
                    checkpoint(statePath, state, `task ${milestoneId}/${phaseId}/${taskId} completed`);
                    if (CONFIG.git.enabled) {
...
// ── Helpers ──────────────────────────────────────────────────
function buildCompletedSiblingsSection(state, milestoneId, phaseId, currentTaskId) {
    const phase = getPhase(state, milestoneId, phaseId);
    const lines = [];
    for (const taskId of sortedKeys(phase.tasks)) {
        if (taskId === currentTaskId)
            break; // Only include tasks before current
        const task = phase.tasks[taskId];
        if (task.status !== "completed")
            continue;
        const planPath = join(CONFIG.pipelineDir, "milestones", milestoneId, "phases", phaseId, "tasks", taskId, "plan-locked.md");
